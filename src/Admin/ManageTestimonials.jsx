    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm">Review, approve, and manage user stories.</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              ...
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase">Story</label>
                <textarea
                  required
                  value={form.story}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {pageItems.map((it) => (
            <div key={it._id} className="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col hover:shadow-md transition-shadow">
              ...
              <div className="p-5 flex-1">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="font-bold text-gray-800 text-sm">{it.name}</h4>
                      <p className="text-xs text-gray-500">{it.role}</p>
                    </div>
                  </div>
                  {/* Status Badge */}
                  <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wide ${
                    it.approved ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"
                  }`}>
                ...
                <div className="relative">
                  <MessageSquare size={16} className="text-gray-300 absolute -top-1 -left-1" />
              ...
              <div className="px-5 py-3 bg-gray-50 border-t border-gray-100 rounded-b-xl flex justify-between items-center">
                <span className="text-xs text-gray-400 flex items-center gap-1">
                  <Clock size={12} /> {new Date(it.createdAt).toLocaleDateString()}
                </span>
