...
function ManageDonations() {
  ...
  const pageItems = filteredDonations.slice((page - 1) * pageSize, page * pageSize);

  // --- UTILS ---
  const handleExport = () => {
    if (!donations.length) return toast.error("No data to export");
    
    const headers = ["ID", "Donor", "Phone", "Amount", "Method", "Purpose", "Status", "Date"];
      ...
      <div className="mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm">Track incoming funds and donor details.</p>
          </div>
          <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
            <div>
              <p className="text-gray-500 text-sm font-medium uppercase">Total Raised</p>
              <h3 className="text-2xl font-bold text-green-600">KES {totalRaised.toLocaleString()}</h3>
            </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-sm divide-y divide-gray-100">
              {pageItems.length === 0 ? (
                ...
                pageItems.map((d) => (
                  <tr key={d._id || d.checkoutRequestID} className="hover:bg-gray-50 transition-colors">
                    <td className="p-4">
                      <div className="font-medium text-gray-800">{d.donorName}</div>
                      <div className="text-xs text-gray-500">{d.phone || "No Phone"}</div>
                    </td>
