...
function ManageProfiles() {
  ...
  const handleSubmit = async (e) => {
    ...
    const toastId = toast.loading(editing ? "Updating profile..." : "Creating profile...");
    ...
    try {
      ...
      toast.success(editing ? "Profile updated" : "Profile created", { id: toastId });
      resetForm();
      fetchProfiles();
    } catch (err) {
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Manage profiles displayed on the public "Meet the Team" page.</p>
        </div>
        <button
          onClick={openCreate}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl animate-in fade-in zoom-in duration-200 my-8">
            <div className="flex justify-between items-center p-5 border-b">
              ...
              <button onClick={resetForm} className="text-gray-400 hover:text-red-500"><X size={24} /></button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2 space-y-4">
                  ...
                  <div className="pt-2">
                    <label className="text-xs font-bold text-gray-500 uppercase mb-2 block">Social Links (Optional)</label>
              ...
              <div className="pt-4 flex justify-end gap-3 border-t">
                ...
                <button 
                  type="submit" 
                  disabled={isSubmitting}
                  className={`px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 shadow flex items-center gap-2 ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''}`}
                >
                  {isSubmitting ? (
                    <>
      ...
      {profiles.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {profiles.map((p) => (
            <div key={p._id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow group">
              {/* Image Header */}
              <div className="h-48 w-full bg-gray-100 relative overflow-hidden">
                ...
                <div className="absolute top-2 right-2">
                  <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wide shadow-sm ${p.approved ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
              ...
              <div className="p-4 text-center">
                ...
                <p className="text-sm text-green-600 font-medium mb-3">{p.role}</p>
