...
function ManageThemes() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(editing ? "Theme updated successfully" : "Theme created successfully");
      resetForm();
      fetchThemes();
    } catch (err) {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Weekly Honor Themes</h2>
          <p className="text-gray-500 text-sm">Manage the daily themes displayed on the "About" page.</p>
        </div>
        <button
          onClick={openNew}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase">Theme Title</label>
                <input
                  required
                  value={form.theme}
              ...
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase mb-2 block">Select Icon</label>
              ...
              <div className="pt-2 flex justify-end gap-3 border-t mt-2">
                ...
                <button type="submit" className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 shadow">Save</button>
              </div>
            </form>
          </div>
        </div>
          ...
          {sortedThemes.map((t) => (
            ...
            >
              ...
              <div className="bg-green-50/50 p-3 border-b border-green-100 flex justify-between items-center">
                <span className="font-bold text-green-700 text-sm tracking-wide">{t.day}</span>
                <span className="text-[10px] text-green-400 bg-white px-1.5 rounded border border-green-100">#{t.order}</span>
              </div>

              {/* Body */}
              <div className="p-4 flex flex-col items-center justify-center text-center flex-grow">
                <div className="text-4xl mb-2 filter drop-shadow-sm">{t.icon}</div>
                <h3 className="font-semibold text-gray-800 text-sm leading-snug">{t.theme}</h3>
              </div>
