...
import { Users, RefreshCcw, LogOut, TrendingUp, BarChart3, Activity } from "lucide-react";
...
const UsersDetails = () => {
  ...
  const fetchData = async (silent = false) => {
    try {
      ...
      const [visitorRes, weeklyRes] = await Promise.all([
        fetch(`${SERVER_URL}/api/visitors/visitor-count`, { headers: { Authorization: `Bearer ${token}` } }),
        fetch(`${SERVER_URL}/api/visitors/weekly-stats`, { headers: { Authorization: `Bearer ${token}` } })
      ]);

      if (!visitorRes.ok || !weeklyRes.ok) throw new Error("Could not sync with server.");
  ...
  if (loading) return <AdminLoader message="Initializing secure analytics..." />;
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-10 px-6 py-4 mb-8">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-slate-800">Analytics Console</h1>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="md:col-span-1 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <div className="flex flex-col h-full justify-between">
              <div>
                <p className="text-slate-500 text-sm font-medium uppercase tracking-wider mb-1">Lifetime Reach</p>
          ...
          <div className="md:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <div className="flex items-center justify-between mb-8">
              ...
              <span className="text-xs text-slate-400 font-medium bg-slate-100 px-2 py-1 rounded">Past 7 Days</span>
            </div>
            ...
            {weeklyData.length > 0 ? (
              ...
              <div className="h-48 flex items-center justify-center border-2 border-dashed border-slate-100 rounded-xl">
                <p className="text-slate-400 text-sm italic">No data synced yet...</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-blue-600 rounded-2xl p-8 text-white relative overflow-hidden shadow-xl shadow-blue-200">
           <div className="relative z-10">
              ...
              <p className="text-blue-100 text-sm max-w-md">
                These insights help you understand when your users are most active. Use this data to schedule updates or major announcements.
              </p>
           </div>
           {/* Decorative circles */}
