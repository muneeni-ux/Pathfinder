...
const Home = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        // Fetch posts where category is "Events"
  ...
  // Helper to format date (e.g., "25 Feb 2025")
  ...
  return (
    <div className="bg-white text-green-900 overflow-x-hidden mt-[-2rem] mb-[-4rem]">
      ...
      {/* 📅 UPCOMING EVENTS (DYNAMIC INTEGRATION) */}
      <section className="py-20 px-6 md:px-12 lg:px-20 bg-green-50 text-center">
        <h2 className="text-3xl md:text-4xl font-bold text-green-900 mb-10">
          Upcoming Events & Announcements
        </h2>
            ...
            events.map((event, index) => (
              ...
              >
                <div className="flex items-center gap-3 text-left">
                  <Calendar className="text-amber-500 flex-shrink-0" />
                  <div>
                    <h4 className="text-green-900 font-semibold line-clamp-1">{event.title}</h4>
                    <p className="text-green-700 text-sm">{formatDate(event.date)}</p>
                  </div>
                </div>
                <Megaphone className="text-green-700 flex-shrink-0 ml-2" />
              </div>
            ))
          ) : (
            <p className="text-green-700 italic">No upcoming events scheduled yet.</p>
          )}
        </div>
        
        {/* Link to view all news/events */}
        <div className="mt-8">
            <Link to="/news-updates" className="text-amber-600 font-semibold hover:text-amber-500 transition">
                View All Updates →
            </Link>
        </div>
      </section>
